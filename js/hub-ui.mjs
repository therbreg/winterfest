...
export function renderOverview(data, state) {
  ...
  <div class="budget-ribbon"><div><p class="folio-label">Budget</p><strong>${euro(summary.budget.planned)} <span>/ ${euro(CAP)} geplant</span></strong></div><div><small>Bereits ausgegeben</small><strong>${euro(summary.budget.actual)}</strong></div><div><small>Prognose gesamt</small><strong>${euro(summary.budget.actual + summary.budget.remaining)}</strong></div><div><small>Reserve</small><strong class="${summary.budget.buffer < 0 ? 'negative' : ''}">${euro(summary.budget.buffer)}</strong></div><button class="text-link" data-go="budget">${summary.budget.unpriced} Posten ohne Preis · Budget öffnen ↗</button></div>
