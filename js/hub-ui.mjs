...
export function renderOverview(data, state) {
  ...
  target.innerHTML = `<section class="milestone-board"><div class="section-caption"><h3>Harte Meilensteine</h3><span>Damit am Eventtag nicht mehr gebaut wird</span></div><div class="milestone-grid">${projectMilestones.map(item=>{const task=summary.tasks.find(row=>row.key===item.id);const done=task&&(task.done||task.status==="done");return `<button class="milestone ${done?"complete":""}" data-go="tasks" data-record="${esc(item.id)}"><time>${esc(item.date)}</time><strong>${esc(item.title)}</strong><span>${done?"Erledigt":"Offen"}</span></button>`;}).join("")}</div></section><div class="command-grid">
    ...
    <section class="command-focus"><p class="folio-label">Aktueller Fokus</p><h3>${esc(summary.focus?.title || (transportPending?'Transportzusage klären':'Bestätigung von Schwamborn abwarten'))}</h3><p>${esc(summary.focus?.text || (transportPending?'Zugfahrzeug und Fahrer verbindlich bestätigen.':'Transport ist geklärt; die Gegenbestätigung von Schwamborn für Angebot und Mietzeitraum steht noch aus.'))}</p>${transportPending?rowLink('Grünschnitt · Transport offen','Aktueller Blocker','tasks','grounds_transport'):''}<button class="text-link" data-go="pins">Fokus & Hinweise ↗</button></section>
...
function renderGrounds(summary,state) {
  ...
  const costRows=costs.map(row=>`<button class="grounds-cost" data-go="assets" data-record="${esc(row.key)}"><span>${esc(row.item||row.key)}</span><strong>${row.key==='grounds_transport'&&!/geklärt|bestätigt/i.test(row.status||'')?'offen':euro(row.planned||0)}</strong><small>${esc(row.status||'Geplant')} · Ist: ${euro(row.actual||0)}</small></button>`).join('');
  ...
  const transportAsset=state.assets?.grounds_transport||{};
  const transportDone=transport?.done||transport?.status==='done'||/geklärt|bestätigt/i.test(transportAsset.status||'');
  target.innerHTML=`<div class="codex-page-heading"><p class="folio-label">Projekt / Gelände & Infrastruktur</p><h2>Erster großer Grünschnitt</h2><p>Grundfreilegung des Grundstücks. Ziel nach dem Wochenende: <strong>Grundfreilegung abgeschlossen</strong> – nicht „Gelände fertig“. Feinschnitt und Nacharbeiten folgen im Januar / Februar 2027.</p><span class="grounds-chip">${firstDone?'Grundfreilegung abgeschlossen':'Ziel: Grundfreilegung'}</span> <span class="grounds-chip">4–5 Helfer</span> <span class="grounds-chip">2 Arbeitstage</span> <span class="grounds-chip">1 AS 901</span> <span class="grounds-chip">2–3 Freischneider</span> <span class="grounds-chip">max. ${euro(FIRST_CUT_RESERVE)}</span></div>
  <div class="grounds-grid"><article class="grounds-card"><h3>${transportDone?'Transport geklärt':'Aktueller Blocker'}</h3><p>${transportDone?'Zugfahrzeug und Fahrer sind zugesagt; der Freund übernimmt die Montagsrückgabe gemeinsam mit dir. Offen bleibt die Gegenbestätigung von Schwamborn.':'<strong>Transportzusage für Zugfahrzeug und Fahrer abwarten.</strong>'}</p>${linked('transport')}${linked('machines')}</article>
  <article class="grounds-card"><h3>Schon entschieden</h3><span class="grounds-chip">Maschinen / Angebot vorhanden</span><span class="grounds-chip">Helfer grundsätzlich geklärt</span><span class="grounds-chip">Arbeitsziel geklärt</span><span class="grounds-chip">Schnittgut geklärt</span><span class="grounds-chip">Sanitär geklärt</span><span class="grounds-chip">Wetterregel geklärt</span><p>AS 901, zwei gemietete Trimmer und eventuell ein weiterer Freischneider. Rollen wechseln. Schnittgut bleibt in wenigen Sammelhaufen außerhalb der Nutzungsflächen. Kein Häcksler beim ersten Einsatz.</p></article>
  ...
  <h3 class="grounds-heading">Timeline</h3><div class="grounds-grid"><article class="grounds-card"><h3>Sofort / etwa eine Woche vorher</h3>${linked('transport')}${linked('machines')}${linked('ppe')}${linked('tools')}${linked('shopping')}</article><article class="grounds-card"><h3>Donnerstag</h3>${linked('thursday')}</article><article class="grounds-card"><h3>Freitag</h3>${linked('friday')}${linked('food')}</article><article class="grounds-card"><h3>Samstag</h3><p>07:30 Silas · 08:00 Schwamborn · 08:45 übrige Helfer · 09:00 Sicherheit · 09:15 Arbeitsbeginn · 12:30 Pizza.</p>${linked('saturday')}${linked('dixi')}</article><article class="grounds-card"><h3>Sonntag</h3><p>Ab 09:00 zweiter Arbeitstag oder Nacharbeit plus Vermessung / Fotos. Geräte bis 16:00–17:00 Uhr reinigen und verladefertig machen.</p>${linked('sunday')}</article><article class="grounds-card"><h3>Montag</h3><p>Kurz vor 08:00 Uhr Rückgabe bei Schwamborn.</p>${linked('monday')}</article></div>
  <h3 class="grounds-heading">Arbeitsablauf</h3><div class="grounds-grid">${day('Grundfreilegung','Reihenfolge nach Zugang und Nutzung.', ['1. Einfahrt freischneiden','2. Grundstückszugang herstellen','3. Eventfläche freilegen','4. Campingfläche freilegen','5. Verbindungswege freilegen','6. Störende Vegetation nur in Nutzungsbereichen zurücknehmen','7. Schnittgut aus Nutzungsflächen räumen','8. Gelände gemeinsam kontrollieren'])}${day('Team im Wechsel','Keine festen dauerhaften Rollen.', ['1 Person am AS 901 Schlegelmäher','2 Personen an gemieteten Freischneidern','Optional zeitweise dritter Freischneider eines Freundes','Übrige Helfer räumen; regelmäßig zwischen Maschinen und Räumen wechseln'])}${day('Schnittgut & Vegetation','Entscheidungen stehen; nur die Position der Haufen wird vor Ort gewählt.', ['Wenige größere Sammelhaufen außerhalb der Nutzungsflächen','Grob nach Grünmaterial, Gestrüpp und Holz trennen','Randvegetation und Baumgruppen möglichst erhalten','Keinen Häcksler einplanen'])}</div>
  <h3 class="grounds-heading">Material, PSA & Verpflegung</h3><div class="grounds-grid"><article class="grounds-card"><h3>Vorhanden / organisiert</h3><p>Festes Schuhwerk, Arbeits-Latzhose / Überziehhose, Gehörschutz, Erste-Hilfe-Kasten, Zeckenzange, Gartenwerkzeuge, Zurrgurte, Benzinkanister, Pavillon und Strom. Schutzbrille / Gesichtsschutz teilweise; Schubkarre wahrscheinlich.</p></article><article class="grounds-card"><h3>Noch prüfen / kaufen</h3><p>PSA-Abfrage; robuste Leder- / Dornenhandschuhe; fehlende Schutzbrillen; nur tatsächlich fehlendes Reinigungs- und Verbrauchsmaterial. Keine Gartenwerkzeuge blind kaufen.</p>${linked('ppe')}${linked('tools')}${linked('shopping')}</article><article class="grounds-card"><h3>Verpflegung & Sanitär</h3><p>Mindestens 12–15 Liter Trinkwasser, Brötchen und vegane Beläge, Snacks sowie ca. 8 einfache Pizzen am Samstagmittag. Kühlbox mit Freitag eingefrorenen Akkus; Quelle nur als Brauchwasser.</p>${linked('food')}${linked('dixi')}</article></div>
  <h3 class="grounds-heading">Maschinen & Kosten</h3><div class="grounds-card"><p><strong>Schwamborn-Angebot zugesagt: rund 496 € brutto, Planwert 500 €</strong> für AS 901 und zwei Trimmer. Die Gegenbestätigung für Samstagmorgen bis Montagmorgen steht noch aus. Bekannte Einzelposten insgesamt: <strong>700–800 €</strong> ohne bezahlten Transport. ${euro(FIRST_CUT_RESERVE)} ist die komfortable Obergrenze / Reserve, keine zusätzliche Ausgabe.</p><div class="grounds-costs">${costRows}</div><button class="text-link" data-go="budget">Gesamtbudget öffnen ↗</button></div>
  <h3 class="grounds-heading">Nach dem Grünschnitt</h3><div class="grounds-grid grounds-next"><article class="grounds-card"><h3>Ergebnis & Geländeplan</h3><p>Dokumentieren, Nutzflächen und Nacharbeiten beurteilen; erst danach den Geländeplan finalisieren.</p>${linked('review')}</article><article class="grounds-card"><h3>Vermessung & 3D-Modell</h3><p>Wenn Sonntag Zeit bleibt beginnen, sonst separat: 20–30 Kontrollmaße, 100–200 überlappende Originalfotos und Übersichtsvideo; danach RealityCapture / RealityScan und Blender.</p>${linked('measure')}${linked('photos')}${linked('model')}${linked('layout')}</article><article class="grounds-card"><h3>Januar / Februar 2027</h3><p>Umfang und Budget bleiben offen, bis Grünschnitt, Vermessung, Modell und Eventlayout ausgewertet sind.</p>${linked('second_cut')}</article></div>`;
}
